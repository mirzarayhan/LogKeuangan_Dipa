...
public class DatabaseHelper extends SQLiteOpenHelper {
    public DatabaseHelper(@Nullable Context context) {
        super(context, "logkeuDB.db", null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("CREATE TABLE session(id integer PRIMARY KEY, login text)");
        sqLiteDatabase.execSQL("CREATE TABLE user(id_user integer PRIMARY KEY AUTOINCREMENT, username text, password text)");
        sqLiteDatabase.execSQL("CREATE TABLE keuangan(id_keuangan integer PRIMARY KEY AUTOINCREMENT, tanggal text, nominal text, keterangan text, kategori text)");
    }
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        ...
        sqLiteDatabase.execSQL("DROP TABLE IF EXISTS keuangan");
